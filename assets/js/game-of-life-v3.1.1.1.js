/*jslint onevar: true, undef: false, nomen: true, eqeqeq: true, plusplus: false, 
 ...
 * http://pmav.eu
 * 04/Sep/2010
 * 29/Mar/2015, make it a circular CA
 */
...
(function () {
  ...
  var GOL = {
    ...
    // Initial state, "row":[columns]
    initialState : '[{"9":[44]},{"10":[42,44]},{"11":[32,33,40,41,54,55]},{"12":[31,35,40,41,54,55]},{"13":[20,21,30,36,40,41]},{"14":[20,21,30,34,36,37,42,44]},{"15":[30,36,44]},{"16":[31,35]},{"17":[32,33]}]',  //Gosper's gun 
...
//    {        columns : 157,  rows : 36,    cellSize : 4},
//   {        columns : 159,  rows : 34,    cellSize : 4},
   {        columns : 159,  rows : 34,    cellSize : 4},
...
/*
 * 
 * when you don't wanna see the buttons
      // Layout
...
//circular
            deadNeighbours = [[(x-1+GOL.columns)%GOL.columns, (y-1+GOL.rows)%GOL.rows, 1], [x%GOL.columns, (y-1+GOL.rows)%GOL.rows, 1], [(x+1)%GOL.columns, (y-1+GOL.rows)%GOL.rows, 1], [(x-1+GOL.columns)%GOL.columns, y%GOL.rows, 1], [(x+1)%GOL.columns, y%GOL.rows, 1], [(x-1+GOL.columns)%GOL.columns, (y+1)%GOL.rows, 1], [x%GOL.columns, (y+1)%GOL.rows, 1], [(x+1)%GOL.columns, (y+1)%GOL.rows, 1]];

//console.log(deadNeighbours); but don't do that unless running it step by step
      ...
      getNeighboursFromAlive : function (x, y, i, possibleNeighboursList) {
		...
		var asl = this.actualState.length; //actual State Length, used in circular CA as we need to mod.
...
//		console.log(this.actualState);
        ...
        // Top
        if (this.actualState[(i-1 +asl)%asl] !== undefined) {
          if (this.actualState[(i-1 +asl)%asl][0] === ((y - 1+GOL.rows)%GOL.rows)) {
//            for (k = this.topPointer; k < this.actualState[(i-1 +asl)%asl].length; k++) {
            for (k = 1; k < this.actualState[(i-1 +asl)%asl].length; k++) {

//              if (this.actualState[i-1][k] >= (x-1+ GOL.columns)%GOL.columns ) {

                if (this.actualState[(i-1 +asl)%asl][k] === ((x - 1+ GOL.columns)%GOL.columns)) {
                  possibleNeighboursList[0] = undefined;
                  this.topPointer = k + 1;
                  neighbours++;
                }

                if (this.actualState[(i-1 +asl)%asl][k] === x) {
                  possibleNeighboursList[1] = undefined;
                  this.topPointer = k;
                  neighbours++;
                }

                if (this.actualState[(i-1 +asl)%asl][k] === ((x + 1)%GOL.columns)) {
      ...
      //          if (this.actualState[i-1][k] > (x + 1)%GOL.columns) {
        //          break;
          //      }
       //       }
       ...
       //   if (this.actualState[i][k] >= (x - 1+ GOL.columns)%GOL.columns) {
            ...
            if (this.actualState[i][k] === ((x + 1)%GOL.columns)) {
        ...
        // Bottom
        if (this.actualState[(i+1)%asl] !== undefined) {
          if (this.actualState[(i+1)%asl][0] === ((y + 1)%GOL.rows)) {
  //          for (k = this.bottomPointer; k < this.actualState[(i+1)%asl].length; k++) {
            for (k = 1; k < this.actualState[(i+1)%asl].length; k++) {
         //     if (this.actualState[i+1][k] >= (x - 1 + GOL.columns)%GOL.columns) {

                if (this.actualState[(i+1)%asl][k] === ((x - 1+ GOL.columns)%GOL.columns)) {
                  possibleNeighboursList[5] = undefined;
                  this.bottomPointer = k + 1;
                  neighbours++;
                }

                if (this.actualState[(i+1)%asl][k] === x) {
                  possibleNeighboursList[6] = undefined;
                  this.bottomPointer = k;
                  neighbours++;
                }

                if (this.actualState[(i+1)%asl][k] === ((x + 1)%GOL.columns)) {
